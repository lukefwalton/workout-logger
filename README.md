...
A local-first iOS workout tracker that doubles as a sandbox for on-device ML,
Apple's Foundation Models, SwiftUI, Swift Charts, and parsing — with a real app
...
This is an incremental build — [`docs/learnings/`](docs/learnings/) records
what each step taught us. The local-first spine is complete and the planned
tracks have shipped; what remains is App Store polish and device soak time.
...
Not yet: cloud anything — and by doctrine, cloud would only ever be an opt-in
experiment, never a tier or a dependency.
...
Set your `DEVELOPMENT_TEAM` in Signing & Capabilities. The app uses minimal,
...
[`lfwdesignsystem/`](lfwdesignsystem), resolved by relative path. It's a copy of
the package shared across the original monorepo — if you change it here, keep the
upstream copy in sync (or vice versa).

## License & attribution

MIT — see [LICENSE](LICENSE), and [NOTICE.md](NOTICE.md) for the reuse log. No
third-party code is bundled; the seed library's muscle vocabulary comes from
the public-domain free-exercise-db. The privacy posture is documented in
[docs/privacy.md](docs/privacy.md). Contributions are welcome under the
[contribution guidelines](CONTRIBUTING.md) with a lightweight
[DCO](DCO.md) sign-off.
