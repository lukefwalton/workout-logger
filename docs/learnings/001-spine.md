...
## Stealing conventions from an earlier app

An earlier LFW iOS app was a clean template. What I adopted:
...
(consistent, not flaky). It's a pre-existing/unrelated failure in unrelated
Python CI; fixing it is out of scope for a Swift-only PR and isn't attempted
here.
