...
enum CardioActivity: CaseIterable {
    ...
    /// SF Symbol for the confirm card / history row. Never emoji (house style).
    var icon: String {
        switch self {
        case .run: "figure.run"
...
/// Shared presentation helpers for a cardio bout — used by the confirm card and
/// the history row so both render duration/distance identically.
enum CardioFormat {
