...
struct TodayView: View {
    ...
    private var modeControl: some View {
        Picker("Input mode", selection: Binding(get: { model.mode }, set: model.setMode)) {
                ...
                HStack(spacing: 12) {
                    ...
                    Button(action: parseInput) {
                        Image(systemName: "arrow.forward")
                            ...
                            .background((canParse && !model.isParsing) ? Theme.ocean.gradient : Theme.steel.opacity(0.25).gradient,
                                        in: Circle())
                    }
                    .disabled(!canParse || model.isParsing)
                    .accessibilityLabel("Parse set")
                }
                .padding(14)
                ...
                HStack(spacing: 12) {
                    ...
                    Button(action: parseInput) {
                        Image(systemName: "arrow.forward")
                            ...
                            .foregroundStyle(.white)
                            .frame(width: 44, height: 44)
                            .background((canParseInPlan && !model.isParsing) ? Theme.ocean.gradient : Theme.steel.opacity(0.25).gradient,
                                        in: Circle())
                    }
                    .disabled(!canParseInPlan || model.isParsing)
                    .accessibilityLabel("Parse planned set")
                }
                .padding(14)
                .background(Theme.paper.opacity(0.9), in: RoundedRectangle(cornerRadius: 18, style: .continuous))
            }
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                VStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        HStack(spacing: 8) {
                            ...
                            Picker("Unit", selection: Binding(get: { model.pendingUnit },
                                                              set: model.setUnit)) {
                ...
                HStack(spacing: 12) {
                    Button("Discard", role: .destructive, action: model.discard)
                        .buttonStyle(.bordered)
                    Button("Save workout", action: model.save)
                        .buttonStyle(.borderedProminent)
